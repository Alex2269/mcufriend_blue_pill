...
mcufriend_blue_pill::mcufriend_blue_pill(int CS, int RS, int WR, int RD, int _RST):Adafruit_GFX(240, 320)
{
...
void mcufriend_blue_pill::reset(void)
{
    done_reset = 1;
    setWriteDir();
    CTL_INIT();
...
void mcufriend_blue_pill::WriteCmdData(uint16_t cmd, uint16_t dat) { writecmddata(cmd, dat); }
...
void mcufriend_blue_pill::pushCommand(uint16_t cmd, uint8_t * block, int8_t N) { WriteCmdParamN(cmd, N, block); }
...
uint16_t mcufriend_blue_pill::readReg(uint16_t reg, int8_t index)
{
    uint16_t ret;
    uint8_t lo;
    if (!done_reset)
...
uint32_t mcufriend_blue_pill::readReg32(uint16_t reg)
{
...
uint32_t mcufriend_blue_pill::readReg40(uint16_t reg)
{
    uint16_t h = readReg(reg, 0);
...
uint16_t mcufriend_blue_pill::readID(void)
{
    uint16_t ret, ret2;
    uint8_t msb;
...
int16_t mcufriend_blue_pill::readGRAM(int16_t x, int16_t y, uint16_t * block, int16_t w, int16_t h)
{
...
void mcufriend_blue_pill::setRotation(uint8_t r)
{
...
void mcufriend_blue_pill::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void mcufriend_blue_pill::setAddrWindow(int16_t x, int16_t y, int16_t x1, int16_t y1)
{
...
void mcufriend_blue_pill::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    int16_t end;
#if defined(SUPPORT_9488_555)
    if (is555) color = color565_to_555(color);
...
void mcufriend_blue_pill::pushColors(uint16_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 0);
}
void mcufriend_blue_pill::pushColors(uint8_t * block, int16_t n, bool first)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, 2);   //regular bigend
}
void mcufriend_blue_pill::pushColors(const uint8_t * block, int16_t n, bool first, bool bigend)
{
    pushColors_any(_MW, (uint8_t *)block, n, first, bigend ? 3 : 1);
}

void mcufriend_blue_pill::vertScroll(int16_t top, int16_t scrollines, int16_t offset)
{
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void mcufriend_blue_pill::invertDisplay(boolean i)
{
    uint8_t val;
...
void mcufriend_blue_pill::begin(uint16_t ID)
{
